...
test('Quant Lab v1 shell and assets are present in the full UI', () => {
  const repoRoot = path.resolve(__dirname, '..');
  const indexHtml = fs.readFileSync(path.join(repoRoot, 'src', 'index.html'), 'utf8');

  assert.match(indexHtml, /data-view="lab"/);
  assert.match(indexHtml, /id="view-lab"/);
  assert.match(indexHtml, /Quant Lab/);
